...
_Last run: **2026-06-21 (Run #67)** — **↧ ▦ ACTIVITY LOAD-ALL DEPTH — the board-wide event feed (⊙ AUTONOMY → ▦ ACTIVITY) could only fetch the newest 100 events, leaving the oldest 26 of the 126-event store permanently unreachable in the UI despite the header reading "100 of 126". A `↧ all {total}` button now pulls the full history on demand.**
